use factory::clean_stats::CleanStats;
use factory::resource::Resource;
use factory::tier::Tier;
...
fn test_resource_tier_assignment() {
    ...
    assert_eq!(resource.tier.unwrap().as_str(), "medium");
    ...
    resource.assign_tier(&[10, 50]);
    assert_eq!(resource.tier.unwrap().as_str(), "high");
}
...
fn test_clean_stats_tier_tracking() {
    ...
    stats.add_stat("kept", Some("low"));
    stats.add_stat("kept", Some("medium"));
    stats.add_stat("kept", Some("high"));
    stats.add_stat("kept", Some("medium"));
...
fn test_clean_stats_unknown_tier() {
    ...
    stats.add_stat("kept", Some("unknown"));
...
fn test_filter_resources_by_tier() {
    ...
    let filtered_resources: Vec<_> = resources
        .into_iter()
        .filter(|r| r.tier.unwrap().as_str() == "low")
        .collect();
    ...
    assert_eq!(result[0].tier.unwrap().as_str(), "low");
}
